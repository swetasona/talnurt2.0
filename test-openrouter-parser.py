...
"""
Test script for the OpenRouter API integration of the Resume Parser
"""
...
os.environ["OPENROUTER_API_TOKEN"] = "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"

print(f"Using OpenRouter API token")
...
def test_parser():
    ...
    print("Testing DeepSeek Resume Parser with OpenRouter API integration")
    print("-" * 80)
    print("Sample resume: python/sample_resume.txt")
    print("Using OpenRouter API token (masked for security)")
    print("Model: deepseek/deepseek-chat-v3-0324:free")
    print("-" * 80)
    ...
    try:
        ...
        if not result.get("success", False):
            ...
            if "API request failed" in error or "OpenRouter API" in error or "Authentication" in error:
                print("\n❌ API authentication error")
                print("👉 The token may not be valid or the API endpoint might be incorrect")
            elif "insufficient balance" in error or "credit" in error:
                print("\n❌ Insufficient balance error")
                print("👉 The OpenRouter token doesn't have enough credits or is not properly configured")
            else:
