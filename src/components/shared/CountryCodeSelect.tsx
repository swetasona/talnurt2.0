...
// Country interface with ISO code
export interface Country {
  name: string;        // Country name
  flag: string;        // Flag emoji
  code: string;        // ISO 2-letter code (e.g., US)
  dial_code: string;   // Phone dial code (e.g., +1)
}

// Country data with codes, names, and flag emoji
...
const CountryCodeSelect: React.FC<CountryCodeSelectProps> = ({
  ...
  // But put United States at the top as it's commonly used
  const usIndex = sortedCountries.findIndex(country => country.name === 'United States');
  if (usIndex !== -1) {
  ...
  return (
    <select
      ...
      onChange={onChange}
      required={required}
      className={`px-3 py-3 rounded-lg border border-gray-200 shadow-sm focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none transition-all bg-white ${className}`}
