...
const AddCandidatePage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle resume file
      if (resumeFile) {
        console.log('Appending resume file to form data:', resumeFile.name);
        formDataToSend.append('resume', resumeFile);
        
        // Double check that the file was appended correctly
        const hasFile = formDataToSend.has('resume');
        console.log('Resume file appended successfully:', hasFile);
      } else {
      ...
      console.log('Sending request to add candidate...');
      
      // Use the add-simple endpoint instead of the employer/candidates/add endpoint
      ...
      if (response.ok) {
        ...
        router.push(`/recruiter/employer/profile-management/candidates/${id}`);
      } else {
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  // Employers should not be able to add candidates
  if (session.user?.role === 'employer') {
    return {
      redirect: {
        destination: '/recruiter/employer/profile-management/candidates',
        permanent: false,
      },
    };
  }
