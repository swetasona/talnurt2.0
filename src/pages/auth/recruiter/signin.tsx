...
import { FaGoogle, FaLinkedin, FaEnvelope, FaLock, FaBriefcase, FaArrowLeft, FaEye, FaEyeSlash } from 'react-icons/fa';
import Navbar from '@/components/Layout/Navbar';
import Footer from '@/components/Layout/Footer';

const RecruiterSignInPage: React.FC = () => {
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      const isRecruiterRole = ['recruiter', 'unassigned', 'employee', 'employer', 'manager'].includes(session.user.role);
      if (isRecruiterRole || session.user.role === 'admin' || session.user.role === 'superadmin') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const callbackUrl = router.query.callbackUrl ? String(router.query.callbackUrl) : '/recruiter/dashboard';
      
      // Note: We don't need to specify a specific role here, as the NextAuth provider will check
      // if the user has any valid recruiter role
      ...
      // Redirect to the callback URL or recruiter dashboard
    ...
    const handleSocialLogin = (provider: string) => {    const callbackUrl = router.query.callbackUrl ? String(router.query.callbackUrl) : '/recruiter/dashboard';    signIn(provider, { callbackUrl: `${callbackUrl}?role=recruiter` });  };
  ...
  return (
    <>
      <Head>
        <title>Recruiter Sign In | Talnurt Recruitment Portal</title>
        <meta name="description" content="Sign in to your recruiter account to post jobs and find the best candidates." />
      </Head>
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-50 via-white to-indigo-100">
        ...
        <main className="flex-1 flex items-center justify-center py-12 md:py-20">
          <section className="w-full md:w-1/2 max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-100 p-8 md:p-10 flex flex-col gap-8 mx-auto">
            <div>
              <h2 className="text-3xl font-extrabold text-gray-900 mb-2 text-center">Sign in to your recruiter account</h2>
              <p className="text-gray-500 mb-6 text-center text-base">Access your hiring dashboard and connect with top talent</p>
            </div>
            <div className="mb-2 text-center">
              ...
              >
                ...
                Looking for a job? Sign in as a candidate
              </Link>
            </div>
            <div className="flex items-center justify-center mb-2 p-3 bg-blue-50 rounded-lg border border-blue-100">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700">Post jobs and manage your hiring process</p>
                </div>
              </div>
            </div>
            {error && (
